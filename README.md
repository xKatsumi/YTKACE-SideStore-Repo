...
- Make both badges use the **same text width** by using a fixed-size badge (same label length visually).
- Add a **line break and spacing** between the two buttons so they stack cleanly on mobile.
- Keep both at the same explicit size.

We’ll:

- Put each button on its own line.
- Add a small paragraph break between them.
- Use the same `width` and `height` on both images.

***

## Updated “Add source” section
...
- Each button is on its own line.
- Added `<br>` between them to ensure a clear break and avoid shifting.
- Both use the same `width="200"` and `height="30"` so they render identically.
- Text is kept as “Add to SideStore” / “Add to LiveContainer” (same character count, so widths match better).

***

## Full `README.md` with this layout

Here’s the complete file ready to paste:
